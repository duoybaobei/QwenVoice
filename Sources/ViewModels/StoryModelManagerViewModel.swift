...
/// Manages install state, download, and delete for the standalone Story Kingdom
/// text LLM.
///
/// This is intentionally separate from `ModelManagerViewModel` (which owns the
/// TTS models): the story writer is a plain text model that must never enter the
/// TTS `models` array, and it is downloaded/unloaded independently so it never
/// stays co-resident with the TTS engine on constrained Macs. The status machine
/// is presence-based (no TTS-style deep integrity / install metadata).
@MainActor
@Observable
final class StoryModelManagerViewModel {
    ...
    /// The story model declared in the contract, or `nil` when the contract has
    ...
    init(
        model: StoryModel? = StoryModelContract.storyModel,
...
extension StoryModelManagerViewModel.DownloadProgress.Phase {
